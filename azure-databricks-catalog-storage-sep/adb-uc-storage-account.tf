...
resource "azurerm_storage_account" "this" {
  ...
  network_rules {
    default_action = "Deny"
    ip_rules = var.storage_account_allowed_ips //e.g terraform environment
  ...
  tags                     = var.tags
  depends_on               = [azurerm_resource_group.this,  data.azurerm_subnet.ws_subnets, data.restapi_object.subnets]
}
